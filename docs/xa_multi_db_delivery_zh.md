...
| remoting XA 分支类型 | `BranchTypeProto` 已显式包含 `XA = 3`，与 XA 分支注册、上报、提交和回滚消息使用的普通协议 `BranchTypeXA` 值对齐；gRPC branch-register、gRPC branch-report、gRPC branch-end 和 Getty branch-end processor 测试均覆盖 XA 映射。 |
