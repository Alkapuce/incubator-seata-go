...
| XA resource contract | MySQL, PostgreSQL, MariaDB, Oracle, and Dameng are covered by shared factory/resource/classifier, recovery-scan, and invalid lifecycle flag contract tests; unsupported DB types keep the default no-op classifier and return a clear resource factory error. |
...
| XA branch type over remoting | `BranchTypeProto` now explicitly includes `XA = 3`, matching the normal protocol `BranchTypeXA` value used by XA branch register, report, commit, and rollback messages; gRPC/Getty branch-register, branch-report, global-lock-query outbound request, and branch-end processor tests cover the XA mapping. |
