...
- 确保由 DSN 派生的 RM resource ID 保持稳定，但不携带 URL userinfo、MySQL/vendor 风格的 `user:password@`，或 `user=`、`password=`、`sslpassword=`、`passfile=` 和 SSL key/certificate 路径等 key-value 凭据；用于稳定识别 resource 的非凭据 key-value 字段应继续保留。
- 增加单元测试，证明 wrapper 返回的 `XAConn` 具有预期 DB type、数据库名、DBType 必填校验、显式或默认 target driver name、parse error 传播时不额外拼接原始 DSN、重复注册错误、resource ID 凭据脱敏，以及带 quoted 或 escaped 分隔符的非凭据 key-value 字段保留行为。
